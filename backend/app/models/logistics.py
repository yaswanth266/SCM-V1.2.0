...
class LogisticsServiceOrder(Base):
    ...
    status = Column(Enum("CREATED", "ACCEPTED", "REJECTED", "IN_PROGRESS", "COMPLETED", "CANCELLED", name="so_status_enum"), default="CREATED", nullable=False)
...
class LogisticsServiceOrderVehicle(Base):
    ...
    vehicle_status = Column(Enum("SCHEDULED", "GATE_IN", "LOADING", "GATE_OUT", "IN_TRANSIT", "TRANSPORTER_ACKNOWLEDGED", "DELIVERY_ACKNOWLEDGED", "CANCELLED", name="so_vehicle_status_enum"), default="SCHEDULED", nullable=False)
...
class DispatchHandover(Base):
    ...
    received_by_emp_code = Column(String(50), nullable=True)
    received_by_aadhar_no = Column(String(50), nullable=True)
    received_by_designation = Column(String(100), nullable=True)
