...
export default function LogisticsDispatch() {
  ...
  const handleDownloadPDF = (mdo) => {
    const w = window.open('', '_blank', 'width=900,height=700');
    if (!w) {
      message.warning('Popup blocked. Allow popups to print/download PDF.');
      return;
    }
    ...
    const materialsRows = (mdo.materials || [])
      .map(
        (mat, idx) => `
        <tr>
          <td>${idx + 1}</td>
          ...
          <td>${mat.quantity} ${mat.unit_of_measure || 'PCS'}</td>
          <td>${mat.batch_number || '-'}</td>
          <td>${mat.number_of_packages || 0}x ${mat.package_type || 'Pallet'}</td>
        </tr>`
      )
      .join('');

    const sdosRows = (mdo.sdos || [])
      .map(
        (sdo, idx) => `
        <tr>
          <td>${sdo.sequence_number || idx + 1}</td>
          <td>${sdo.sdo_number || '-'}</td>
          <td>${sdo.custodian_position_name || '-'}</td>
          <td>${sdo.status || '-'}</td>
          <td>${sdo.received_by_name || '-'}</td>
          <td>${sdo.received_at ? formatDate(sdo.received_at) : '-'}</td>
        </tr>`
      )
      .join('');
    ...
    w.document.write(`
      <html>
        <head>
          <title>Dispatch Plan - ${mdo.mdo_number}</title>
          <style>
            body { font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1e293b; padding: 30px; font-size: 13px; line-height: 1.5; }
            .header { text-align: center; border-bottom: 2px solid #0f172a; padding-bottom: 15px; margin-bottom: 20px; }
            .company-title { font-size: 20px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: #0f172a; margin: 0; }
            .doc-title { font-size: 14px; font-weight: 600; color: #64748b; margin-top: 5px; margin-bottom: 0; text-transform: uppercase; }
            ...
            .section-title { font-weight: 700; text-transform: uppercase; font-size: 11px; color: #475569; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px; margin-top: 0; margin-bottom: 10px; }
            .meta-list { list-style: none; padding: 0; margin: 0; }
            .meta-list li { margin-bottom: 6px; display: flex; justify-content: space-between; }
            .meta-list li span { font-weight: 600; color: #64748b; }
            ...
            .footer-sigs { margin-top: 60px; display: flex; justify-content: space-between; flex-wrap: wrap; }
            .signature-box { border-top: 1px solid #94a3b8; width: 220px; text-align: center; padding-top: 5px; font-size: 11px; color: #64748b; margin-top: 40px; }
            @media print {
              body { padding: 0; }
        ...
        <body>
          <div class="no-print" style="margin-bottom: 20px; text-align: right;">
            <button onclick="window.print()" style="padding: 8px 16px; background: #0284c7; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 13px;">Print / Save as PDF</button>
          </div>

          <div class="header">
            <div class="company-title">Bhavya SCM Logistics</div>
            <div class="doc-title">Dispatch Plan Manifest</div>
          </div>
          ...
          <div class="grid">
            <div class="section">
              ...
              <ul class="meta-list">
                <li><span>Dispatch Plan No:</span> <strong>${mdo.mdo_number || '-'}</strong></li>
                <li><span>Store Origin:</span> <strong>${mdo.warehouse_name || '-'}</strong></li>
                <li><span>Dispatch Date:</span> <strong>${formatDate(mdo.order_date)}</strong></li>
                <li><span>Required Delivery:</span> <strong>${formatDate(mdo.required_delivery_date)}</strong></li>
                <li><span>Dispatch Mode:</span> <strong>${(mdo.dispatch_mode || 'direct').toUpperCase()}</strong></li>
                <li><span>Dispatch Type:</span> <strong>${(mdo.dispatch_type || 'THIRD_PARTY').toUpperCase().replace('_', ' ')}</strong></li>
                <li><span>Status:</span> <strong>${mdo.status || '-'}</strong></li>
                <li><span>Priority:</span> <strong>${mdo.priority || '-'}</strong></li>
              </ul>
            </div>

            <div class="section">
              <div class="section-title">Transit Route & Logistics</div>
              <ul class="meta-list">
                <li><span>Pick Up Location:</span> <strong>${addr.pickup || 'Origin Warehouse'}</strong></li>
                <li><span>Drop Off Location:</span> <strong>${addr.dropoff || 'Destination Warehouse'}</strong></li>
                <li><span>Logistics Weight:</span> <strong>${inst.weight || parseFloat(mdo.total_weight_kg || 0)} KG</strong></li>
                <li><span>Logistics Volume:</span> <strong>${inst.volume || parseFloat(mdo.total_volume_cft || 0)} CFT</strong></li>
                <li><span>Items Description:</span> <strong>${inst.desc || '-'}</strong></li>
                <li><span>Reference Indent:</span> <strong>${mdo.indent_number || '-'}</strong></li>
                <li><span>Reference Issue:</span> <strong>${mdo.material_issue_number || '-'}</strong></li>
              </ul>
            </div>
          </div>
          ...
          ${
            ...
            <div class="section" style="margin-bottom: 20px;">
              <div class="section-title">Handover & Carrier Details</div>
              <div class="grid" style="grid-template-columns: repeat(3, 1fr); gap: 15px; background: transparent; border: none; padding: 0; margin: 0;">
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">CARRIER / COURIER</span>
                  <strong>${mdo.handover.courier_name || '-'}</strong>
                </div>
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">AWB / TRACKING NO</span>
                  <strong>${mdo.handover.awb_no || '-'}</strong>
                </div>
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">VEHICLE NUMBER</span>
                  <strong>${mdo.handover.vehicle_no || '-'}</strong>
                </div>
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">DRIVER NAME</span>
                  <strong>${mdo.handover.driver_name || '-'}</strong>
                </div>
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">DRIVER PHONE</span>
                  <strong>${mdo.handover.driver_phone || '-'}</strong>
                </div>
                <div>
                  <span style="font-size: 11px; color: #64748b; display: block;">HANDOVER REMARKS</span>
                  <strong>${mdo.handover.remarks || '-'}</strong>
                </div>
              </div>
            </div>`
              : ''
          ...
          <table>
            ...
            <tbody>
              ${materialsRows || '<tr><td colspan="6" style="text-align:center">No materials listed</td></tr>'}
              <tr class="total-row">
                <td colspan="3">TOTAL</td>
                <td>${mdo.materials ? mdo.materials.reduce((acc, m) => acc + m.quantity, 0) : 0}</td>
                <td>-</td>
                <td>${mdo.materials ? mdo.materials.reduce((acc, m) => acc + (m.number_of_packages || 0), 0) : 0} Packages</td>
              </tr>
            </tbody>
          </table>

          ${
            mdo.sdos && mdo.sdos.length > 0
              ? `
            <h3>Custody Transfer Chain (Sub Dispatch Orders)</h3>
            <table>
              <thead>
                <tr>
                  <th>Seq</th>
