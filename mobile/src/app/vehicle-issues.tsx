...
export default function VehicleIssuesScreen() {
  ...
  const fetchMasters = async (apiBase: string, authToken: string) => {
    try {
      ...
      const [whRes, projRes, vehRes, itemRes, usrRes, indRes] = await Promise.all([
        ...
        axios.get(`${apiBase}/api/v1/masters/vehicles`, { headers, params: { page_size: 100 } }),
  ...
  const fetchIssues = async (apiBase: string, authToken: string, pageNum: number, searchQuery: string) => {
    try {
      ...
      console.error(e);
      Alert.alert('Error', 'Failed to retrieve vehicle material issues.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const loadMore = () => {
    if (issues.length < total && !loading) {
  ...
  const handleConfirmIssue = async (issueId: number) => {
    try {
      await axios.post(`${API_BASE_URL}/api/v1/warehouse/vehicle-issues/${issueId}/issue`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      Alert.alert('Success', 'Vehicle issue confirmed and marked as Issued!');
      setDetailModalVisible(false);
      handleRefresh();
    } catch (err: any) {
  ...
  const handleSubmitForm = async () => {
    ...
    if (!selectedVehicleCode) {
      Alert.alert('Validation Error', 'Please select a vehicle code.');
      return;
    }
    if (formItems.length === 0) {
    ...
    const finalVehicleNumber = (vehicleNumber || selectedVehicleCode || '-').trim();
    ...
    try {
      ...
      Alert.alert('Submission Failed', formatApiError(err, 'Failed to create vehicle issue.'));
    } finally {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.headerTop}>
          ...
          <Text style={styles.headerTitle}>Vehicle Material Issues</Text>
      ...
      <View style={styles.searchBarContainer}>
        <TextInput
          ...
          placeholder="Search issue # or vehicle code..."
          placeholderTextColor="#94A3B8"
          value={search}
          onChangeText={(val) => {
            setSearch(val);
      ...
      {loading && page === 1 ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading vehicle issues...</Text>
        </View>
      ) : issues.length === 0 ? (
        <ScrollView
        ...
        >
          ...
          <Text style={styles.emptyText}>No vehicle issues found</Text>
          <TouchableOpacity style={styles.emptyButton} onPress={openNewForm}>
            <Text style={styles.emptyButtonText}>Create Vehicle Issue</Text>
          </TouchableOpacity>
        </ScrollView>
      ) : (
        <FlatList
      ...
      >
        <SafeAreaView style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <Text style={styles.modalTitle}>Vehicle Issue Details</Text>
            <View style={{ width: 20 }} />
          </View>
      ...
      >
        <SafeAreaView style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <Text style={styles.modalTitle}>New Vehicle Issue</Text>
            <View style={{ width: 20 }} />
          </View>
          ...
          {formLoading ? (
            <View style={styles.loadingContainer}>
              <ActivityIndicator size="large" color="#481238" />
              <Text style={styles.loadingText}>Saving vehicle issue...</Text>
            </View>
          ) : (
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              >
                <Text style={styles.submitBtnText}>Create Vehicle Issue</Text>
              </TouchableOpacity>
            </ScrollView>
          )}
